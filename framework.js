! function() {
    var config = (0 === window.location.protocol.indexOf("https"), { kitId: "zam5nzz", host: "https://typeface.nyt.com/", scriptTimeout: 3e3 }),
    ...
    var t = setTimeout(function() { h.className = h.className.replace(/(\s|^)wf-loading(\s|$)/g, " "), h.className += " wf-inactive" }, config.scriptTimeout),
        ...
        d = !1;
    tk.src = config.host + config.kitId + ".js", tk.type = "text/javascript", tk.async = "true", tk.onload = tk.onreadystatechange = function() { var a = this.readyState; if (!(d || a && "complete" != a && "loaded" != a)) { d = !0, clearTimeout(t); try { Typekit.load(config) } catch (b) {} } };
    var s = document.getElementsByTagName("script")[0];
    s.parentNode.insertBefore(tk, s)
}(), window.NYTABTEST = {}, window.NYTABTEST.config = function() {
        "use strict";

        function Test(config) { var prop; for (prop in config) config.hasOwnProperty(prop) && (this[prop] = config[prop]) }
        var i, configs, tests = [],
            configDOMElement = document.getElementById("abtestconfig");
        if (configDOMElement) try { configs = JSON.parse(configDOMElement.innerHTML) } catch (e) { configs = [], window.NYTABTEST.configDOMElement = null } else configs = [], window.NYTABTEST.configDOMElement = null;
        for (Test.prototype = {
                STATUS: { waiting: "-3", excluded: "-2", throttle: "-1", control: "0", variantA: "1", variantB: "2", variantC: "3", variantD: "4", variantE: "5", variantF: "6", variantG: "7", variantH: "8", variantI: "9", variantJ: "10", variantK: "11", variantL: "12", variantM: "13", variantN: "14", variantO: "15", variantP: "16", variantQ: "17", variantR: "18", variantS: "19", variantT: "20", variantU: "21", variantV: "22", variantW: "23", variantX: "24", variantY: "25", variantZ: "26" },
                userStatus: "-3",
                getTestId: function() { return this.testId },
                getTestName: function() { return this.testName },
                getThrottle: function() { return parseFloat(this.throttle) },
                getAllocation: function() { return parseFloat(this.allocation) },
                getUserStatus: function() { return this.userStatus },
                hasIsEnabledProperty: function() { return this.hasOwnProperty("isEnabled") },
                isUserThrottle: function() { return parseInt(this.userStatus, 10) === parseInt(this.STATUS.throttle, 10) },
                isUserVariant: function() { return parseInt(this.userStatus, 10) >= parseInt(this.STATUS.throttle, 10) ? this.userStatus : "" },
                isUserInAnyVariant: function() { return parseInt(this.userStatus, 10) >= parseInt(this.STATUS.variantA, 10) },
                isUserControl: function() { return parseInt(this.userStatus, 10) === parseInt(this.STATUS.control, 10) },
                getRandomNumber: function() { return Math.random() },
                determineVariant: function() {
                    var min = 1,
                        max = parseInt(this.variants, 10);
                    this.userStatus = Math.floor(Math.random() * (max - min + 1) + min).toString()
                },
                determine: function(force) { "undefined" != typeof force ? this.userStatus = force : this.userStatus === this.STATUS.waiting ? this.getRandomNumber() <= parseFloat(this.throttle) ? (this.userStatus = this.STATUS.throttle, this.determine()) : this.userStatus = this.STATUS.excluded : this.userStatus === this.STATUS.throttle && (this.getRandomNumber() <= parseFloat(this.allocation) ? this.determineVariant() : this.userStatus = this.STATUS.control) }
        ...
        var engine, tests = window.NYTABTEST.config,
            ...
            writeCookie = function(value) {
                var now, options = { expires: 365, domain: ".nytimes.com", path: "/", secure: !1 };
        ...
        return engine = {
            getCookieName: function() { return cookieName },
            eachTest: function(iterator) { var i; for (i = 0; i < tests.length; i += 1) iterator(tests[i], i) },
            writeStatusToCookie: function() {
                var that = this,
                    cookieValue = [];
                this.eachTest(function(test) { test.hasIsEnabledProperty() ? test.isEnabled && (that.isTestAllocated(test) || !that.isTestAllocated(test) && that.isAppIncludedInTest(test)) && cookieValue.push(test.getTestId() + "|" + test.getUserStatus()) : (that.isTestAllocated(test) || !that.isTestAllocated(test) && that.isAppIncludedInTest(test)) && cookieValue.push(test.getTestId() + "|" + test.getUserStatus()) }), writeCookie(cookieValue.length > 0 ? cookieValue.join("&") : null)
            },
            readStatusFromCookie: function() {
                var i, cookieValue, cookieValueArray;
                if (cookiesEnabled())
                    for (cookieValue = readCookie(), cookieValueArray = cookieValue.length > 0 ? cookieValue.split("&") : [], i = 0; i < cookieValueArray.length; i += 1) cookieValue = cookieValueArray[i].split("|"), this.eachTest(function(test) { test.getTestId() === cookieValue[0] && (testsInCookie.push(cookieValue[0]), test.determine(cookieValue[1])) });
                else this.eachTest(function(test) { test.determine(test.STATUS.excluded) })
            },
            determineStatus: function() { this.eachTest(function(test) { test.determine() }) },
            getApplicationName: function() { var meta; return "undefined" == typeof this.applicationName && (meta = document.getElementById("applicationName"), this.applicationName = meta ? meta.getAttribute("content") : ""), this.applicationName },
            isTestAllocated: function(test) { var id = test.testId; return testsInCookie.indexOf(id) > -1 },
            ...
            findTest: function(query) { var found, that = this; return this.eachTest(function(test) { found || test.getTestId() !== query && test.getTestName() !== query || !that.isAppIncludedInTest(test) || (found = test) }), found },
            getClassNamesForVariants: function() {
                var names = [],
                    that = this;
                return this.eachTest(function(test) { test.isUserVariant() && that.isAppIncludedInTest(test) && names.push(test.getTestName() + "-" + test.getUserStatus()) }), names.join(" ")
            },
            isUserVariant: function(query) { var test = this.findTest(query); return !!test && test.isUserVariant() },
            isUserInAnyVariant: function(query) { var test = this.findTest(query); return !!test && test.isUserInAnyVariant() },
            isUserControl: function(query) { var test = this.findTest(query); return !!test && test.isUserControl() },
            isUserThrottle: function() { var isThrottle = !1; return this.eachTest(function(test) {!isThrottle && test.isUserThrottle() && (isThrottle = !0) }), isThrottle },
            toObject: function() { var obj = {}; return this.eachTest(function(test) { test.isUserThrottle() && (obj[test.getTestId()] = test.getUserStatus()) }), obj },
        ...
        if (configDOMElement) try { config = JSON.parse(configDOMElement.innerHTML), handleOverrides() } catch (e) {}
        return config
    }(), window.NYTEXPERIMENTS.engine = function() {
        "use strict";
        var engine, config = window.NYTEXPERIMENTS.config;
        return engine = {
            findExperiment: function(experiment) { return config[experiment] },
            eachExperiment: function(iterator) { var experiment; for (experiment in config) iterator(experiment) },
            getClassNamesForExperiments: function() {
                var names = [],
                    that = this;
                return this.eachExperiment(function(experiment) { that.isExperiment(experiment) >= 0 && names.push(experiment + "-" + that.findExperiment(experiment).value) }), names
            },
            isExperiment: function(query) { var value, experiment = this.findExperiment(query); return experiment ? value = parseInt(experiment.value, 10) : -1 }
        }
        ...
        var classLength, device, orientationType, viewportClass, eventQueue = {},
            ...
            breakpoints = [{ key: "small", val: 315 }, { key: "small-10", val: 450 }, { key: "small-20", val: 600 }, { key: "medium", val: 720 }, { key: "medium-10", val: 765 }, { key: "medium-20", val: 855 }, { key: "medium-30", val: 960 }, { key: "medium-31", val: 975 }, { key: "medium-40", val: 1005 }, { key: "medium-50", val: 1020 }, { key: "medium-60", val: 1050 }, { key: "large", val: 1080 }, { key: "large-10", val: 1125 }, { key: "large-11", val: 1155 }, { key: "large-20", val: 1200 }, { key: "large-21", val: 1215 }, { key: "large-30", val: 1245 }, { key: "large-40", val: 1280 }, { key: "large-41", val: 1335 }, { key: "large-50", val: 1375 }, { key: "large-51", val: 1410 }, { key: "large-60", val: 1607 }, { key: "large-70", val: 1650 }, { key: "large-80", val: 2030 }],
            getWidth = function() {
                var func;
                if (document.documentElement && document.documentElement.clientWidth) func = function() { return document.documentElement.clientWidth };
                else if (window.self && window.self.innerWidth) func = function() { return window.self.innerWidth };
                else {
                    if (!document.body) return function() { return 0 };
                    func = function() { return document.body.clientWidth }
                }
                return func
            }(),
            getHeight = function() {
                var func;
                if (document.documentElement && document.documentElement.clientHeight) func = function() { return document.documentElement.clientHeight };
                else if (window.self && window.self.innerHeight) func = function() { return window.self.innerHeight };
                else {
                    if (!document.body) return function() { return 0 };
                    func = function() { return document.body.clientHeight }
                }
                return func
            }(),
            createTrigger = function(eventName) { return function() { var i, callbacks = eventQueue[eventName].callbacks; for (i = 0; i < callbacks.length; i += 1) callbacks[i].apply({}, arguments) } },
            on = function(eventName, callback) { eventQueue.hasOwnProperty(eventName) ? eventQueue[eventName].callbacks.push(callback) : eventQueue[eventName] = { callbacks: [callback], trigger: createTrigger(eventName) } },
            getViewport = function() { return viewportClass },
            getViewportInteger = function() {
                var numberString = viewportClass.match(/\d+/),
                    ...
                    sizes = { small: 1, medium: 10, large: 100 };
                return parseInt(sizes[match] + "" + number, 10)
            },
            setClasses = function(newClasses) { rootEl.className = newClasses },
            getClasses = function() { return rootEl.className },
            addClass = function(newClass) { setClasses(newClass + " " + getClasses()) },
            ...
            getOrientation = function() { return orientationType },
            ...
            getFlags = function() { return featureFlags },
        ...
        var bleedType, coverImage, coverImageHeight, coverImageWidth, coverImageCaption, story, storyHeader, headline, storyMeta, storyBody, cover, coverHeight, realImgHeight, realImgWidth, imageAspectRatio, imageOrientation, topCropBias, leftCropBias, bottomCropBias, rightCropBias, coverImageCaptionHeight, backgroundColor, shell, masthead, html, windowWidth = window.magnum.getWidth(),
            ...
            setStoryBodyTop = function(top) { "full-bleed-cover" !== bleedType || belowCoverToLedeBreakpoint ? "vertical-full-bleed" === bleedType ? coverImageCaptionHeight > 0 ? isBelowVerticalFullBleedBreakpoint || (top += coverImageCaptionHeight + 65) : top += 80 : top += 20 : top += coverImageCaptionHeight > 0 ? coverImageCaptionHeight + 65 : 80, storyBody.style.marginTop = top + "px" },
            setCaptionPosition = function(newCaptionPosition) { coverImageCaption && (coverImageCaption.style.top = newCaptionPosition + 7 + "px") },
            setCoverHeight = function(newCoverHeight, newCaptionPosition) { cover.style.height = newCoverHeight + "px", setCaptionPosition(newCaptionPosition) },
            setBelowCoverToLedeBreakpoint = function(value) { belowCoverToLedeBreakpoint = value },
            setBelowVerticalFullBleedBreakpoint = function(value) { isBelowVerticalFullBleedBreakpoint = value },
            ...
            getAllDimensions = function() { windowHeight = window.magnum.getHeight(), windowWidth = window.magnum.getWidth(), coverHeight = cover.clientHeight, coverWidth = cover.clientWidth, coverImageHeight = coverImage.clientHeight, coverImageWidth = coverImage.clientWidth, coverImageCaptionHeight = coverImageCaption ? coverImageCaption.clientHeight : 0 },
            ...
            makeFullBleedCover = function() { getAllDimensions(), setCoverHeight(windowHeight, windowHeight), setStoryBodyTop(windowHeight), fitImage() },
            makeVerticalFullBleed = function() { getAllDimensions(), setCoverHeight(windowHeight, windowHeight), "transparent" !== storyMeta.style.backgroundColor && storyMeta.style.backgroundColor || (storyMeta.style.backgroundColor = backgroundColor), handleMastheadClasses(), setStoryBodyTop(windowHeight), fitImage() },
            handleMastheadClasses = function() { "headline-image-topper" === bleedType ? masthead && masthead.className.indexOf("masthead-headline-image-topper") === -1 && (masthead.className += " masthead-headline-image-topper") : "vertical-full-bleed" === bleedType && masthead && masthead.className.indexOf("masthead-vertical-fullbleed") === -1 && (masthead.className += " masthead-vertical-fullbleed", document.getElementById("story").className.indexOf("bleed-align-left") !== -1 && (masthead.className += " bleed-align-left"), html.className.indexOf("section-magazine") !== -1 && (masthead.className += " masthead-magazine"), html.className.indexOf("section-t-magazine") !== -1 && (masthead.className += " masthead-t-mag")) },
            makeShallowFullBleedLede = function(imageHeight, coverWidth) { 0 === imageHeight && (imageHeight = coverWidth / imageAspectRatio), setCoverHeight(imageHeight + coverImageCaptionHeight, imageHeight), handleMastheadClasses(), setStoryBodyTop(storyHeader.clientHeight) },
        ...
        function G(a, b) { for (var d in a) { var e = a[d]; if (!F(e, "-") && j[e] !== c) return "pfx" != b || e } return !1 }
        ...
        var x, B, d = "2.8.3",
            ...
            A = {}.hasOwnProperty;
        B = E(A, "undefined") || E(A.call, "undefined") ? function(a, b) { return b in a && E(a.constructor.prototype[b], "undefined") } : function(a, b) { return A.call(a, b) }, Function.prototype.bind || (Function.prototype.bind = function(b) {
            var c = this;
        ...
        }, s.cssreflections = function() { return I("boxReflect") }, s.csstransforms = function() { return !!I("transform") }, s.csstransforms3d = function() { var a = !!I("perspective"); return a && "webkitPerspective" in g.style && y("@media (transform-3d),(-webkit-transform-3d){#modernizr{left:9px;position:absolute;height:3px;}}", function(b, c) { a = 9 === b.offsetLeft && 3 === b.offsetHeight }), a }, s.csstransitions = function() { return I("transition") }, s.fontface = function() {
            var a;
        ...
        return e.input || J(), e.addTest = function(a, b) {
            ...
            return e
        }, C(""), i = k = null, e._version = d, e._prefixes = n, e._domPrefixes = q, e._cssomPrefixes = p, e.hasEvent = z, e.testProp = function(a) { return G([a]) }, e.testAllProps = I, e.testStyles = y, g.className = g.className.replace(/(^|\s)no-js(\s|$)/, "$1$2") + (f ? " has-js has-" + v.join(" has-") : ""), e
    }(this, this.document),
    function(a, b) {
        function l(a, b) {
        ...
        function w(a) { for (var b, c = a.getElementsByTagName("*"), d = c.length, e = RegExp("^(?:" + m().join("|") + ")$", "i"), f = []; d--;) b = c[d], e.test(b.nodeName) && f.push(b.applyElement(x(b))); return f }

        function x(a) { for (var b, c = a.attributes, d = c.length, e = a.ownerDocument.createElement(u + ":" + a.nodeName); d--;) b = c[d], b.specified && e.setAttribute(b.nodeName, b.nodeValue); return e.style.cssText = a.style.cssText, e }

        function y(a) { for (var b, c = a.split("{"), d = c.length, e = RegExp("(^|[\\s,>+~])(" + m().join("|") + ")(?=[[\\s,>+~#.:]|$)", "gi"), f = "$1" + u + "\\:$2"; d--;) b = c[d] = c[d].split("}"), b[b.length - 1] = b[b.length - 1].replace(e, f), c[d] = b.join("}"); return c.join("{") }

        function z(a) { for (var b = a.length; b--;) a[b].removeNode() }
        ...
        ! function() {
            try {
                ...
                a.innerHTML = "<xyz></xyz>", g = "hidden" in a, k = 1 == a.childNodes.length || function() { b.createElement("a"); var a = b.createDocumentFragment(); return "undefined" == typeof a.cloneNode || "undefined" == typeof a.createDocumentFragment || "undefined" == typeof a.createElement }()
            } catch (c) { g = !0, k = !0 }
        }();
        ...
        var t = /^$|\b(?:all|print)\b/,
            u = "html5shiv",
            v = !k && function() { var c = b.documentElement; return "undefined" != typeof b.namespaces && "undefined" != typeof b.parentWindow && "undefined" != typeof c.applyElement && "undefined" != typeof c.removeNode && "undefined" != typeof a.attachEvent }();
...
! function(global) {
    function isFunction(it) { return "[object Function]" === ostring.call(it) }

    function isArray(it) { return "[object Array]" === ostring.call(it) }

    function each(ary, func) { if (ary) { var i; for (i = 0; i < ary.length && (!ary[i] || !func(ary[i], i, ary)); i += 1); } }

    function eachReverse(ary, func) { if (ary) { var i; for (i = ary.length - 1; i > -1 && (!ary[i] || !func(ary[i], i, ary)); i -= 1); } }
    ...
    function mixin(target, source, force, deepStringMixin) { return source && eachProp(source, function(value, prop) {!force && hasProp(target, prop) || (deepStringMixin && "string" != typeof value ? (target[prop] || (target[prop] = {}), mixin(target[prop], value, force, deepStringMixin)) : target[prop] = value) }), target }

    function bind(obj, fn) { return function() { return fn.apply(obj, arguments) } }

    function scripts() { return document.getElementsByTagName("script") }

    function getGlobal(value) { if (!value) return value; var g = global; return each(value.split("."), function(part) { g = g[part] }), g }
    ...
    function newContext(contextName) {
                ...
                for (nameParts = name.split("/"), i = nameParts.length; i > 0; i -= 1) {
                    if (nameSegment = nameParts.slice(0, i).join("/"), baseParts)
                        for (j = baseParts.length; j > 0; j -= 1)
                            if (mapValue = getOwn(map, baseParts.slice(0, j).join("/")), mapValue && (mapValue = getOwn(mapValue, nameSegment))) { foundMap = mapValue, foundI = i; break }
                    if (foundMap) break;
        ...
        function removeScript(name) { isBrowser && each(scripts(), function(scriptNode) { if (scriptNode.getAttribute("data-requiremodule") === name && scriptNode.getAttribute("data-requirecontext") === context.contextName) return scriptNode.parentNode.removeChild(scriptNode), !0 }) }

        function hasPathFallback(id) { var pathConfig = getOwn(config.paths, id); if (pathConfig && isArray(pathConfig) && pathConfig.length > 1) return removeScript(id), pathConfig.shift(), context.require.undef(id), context.require([id]), !0 }
        ...
        function takeGlobalQueue() { globalDefQueue.length && (apsp.apply(defQueue, [defQueue.length - 1, 0].concat(globalDefQueue)), globalDefQueue = []) }
        ...
        function checkLoaded() {
            ...
            if (!inCheckLoaded) {
                ...
                needCycleCheck && each(reqCalls, function(mod) { breakCycle(mod, {}, {}) }), expired && !usingPathFallback || !stillLoading || !isBrowser && !isWebWorker || checkLoadedTimeoutId || (checkLoadedTimeoutId = setTimeout(function() { checkLoadedTimeoutId = 0, checkLoaded() }, 50)), inCheckLoaded = !1
            }
        }

        function callGetModule(args) { hasProp(defined, args[0]) || getModule(makeModuleMap(args[0], null, !0)).init(args[1], args[2]) }
        ...
        function getScriptData(evt) { var node = evt.currentTarget || evt.srcElement; return removeListener(node, context.onScriptLoad, "load", "onreadystatechange"), removeListener(node, context.onScriptError, "error"), { node: node, id: node && node.getAttribute("data-requiremodule") } }
        ...
        var inCheckLoaded, Module, context, handlers, checkLoadedTimeoutId, config = { waitSeconds: 7, baseUrl: "./", paths: {}, pkgs: {}, shim: {}, map: {}, config: {} },
            registry = {},
            undefEvents = {},
            defQueue = [],
            defined = {},
        ...
        return handlers = { require: function(mod) { return mod.require ? mod.require : mod.require = context.makeRequire(mod.map) }, exports: function(mod) { if (mod.usingExports = !0, mod.map.isDefine) return mod.exports ? mod.exports : mod.exports = defined[mod.map.id] = {} }, module: function(mod) { return mod.module ? mod.module : mod.module = { id: mod.map.id, uri: mod.map.url, config: function() { return config.config && getOwn(config.config, mod.map.id) || {} }, exports: defined[mod.map.id] } } }, Module = function(map) { this.events = getOwn(undefEvents, map.id) || {}, this.map = map, this.shim = getOwn(config.shim, map.id), this.depExports = [], this.depMaps = [], this.depMatched = [], this.pluginMaps = {}, this.depCount = 0 }, Module.prototype = {
            init: function(depMaps, factory, errback, options) { options = options || {}, this.inited || (this.factory = factory, errback ? this.on("error", errback) : this.events.error && (errback = bind(this, function(err) { this.emit("error", err) })), this.depMaps = depMaps && depMaps.slice(0), this.errback = errback, this.inited = !0, this.ignore = options.ignore, options.enabled || this.enabled ? this.enable() : this.check()) },
            defineDep: function(i, depExports) { this.depMatched[i] || (this.depMatched[i] = !0, this.depCount -= 1, this.depExports[i] = depExports) },
            fetch: function() { if (!this.fetched) { this.fetched = !0, context.startTime = (new Date).getTime(); var map = this.map; return this.shim ? void context.makeRequire(this.map, { enableBuildCallback: !0 })(this.shim.deps || [], bind(this, function() { return map.prefix ? this.callPlugin() : this.load() })) : map.prefix ? this.callPlugin() : this.load() } },
            load: function() {
                    ...
                    if (this.inited) {
                        if (this.error) this.emit("error", this.error);
                            ...
                            if (this.defining = !0, this.depCount < 1 && !this.defined) {
                                if (isFunction(factory)) { if (this.events.error) try { exports = context.execCb(id, factory, depExports, exports) } catch (e) { err = e } else exports = context.execCb(id, factory, depExports, exports); if (this.map.isDefine && (cjsModule = this.module, cjsModule && void 0 !== cjsModule.exports && cjsModule.exports !== this.exports ? exports = cjsModule.exports : void 0 === exports && this.usingExports && (exports = this.exports)), err) return err.requireMap = this.map, err.requireModules = [this.map.id], err.requireType = "define", onError(this.error = err) } else exports = factory;
            ...
            enable: function() {
                this.enabled = !0, this.enabling = !0, each(this.depMaps, bind(this, function(depMap, i) {
                    ...
                    if ("string" == typeof depMap) {
                        ...
                        this.depCount += 1, on(depMap, "defined", bind(this, function(depExports) { this.defineDep(i, depExports), this.check() })), this.errback && on(depMap, "error", this.errback)
                    }
            ...
            emit: function(name, evt) { each(this.events[name], function(cb) { cb(evt) }), "error" === name && delete this.events[name] }
        }, context = {
            config: config,
            ...
            configure: function(cfg) {
                ...
                eachProp(cfg, function(value, prop) { objs[prop] ? "map" === prop ? mixin(config[prop], value, !0, !0) : mixin(config[prop], value, !0) : config[prop] = value }), cfg.shim && (eachProp(cfg.shim, function(value, id) { isArray(value) && (value = { deps: value }), !value.exports && !value.init || value.exportsFn || (value.exportsFn = context.makeShimExports(value)), shim[id] = value }), config.shim = shim), cfg.packages && (each(cfg.packages, function(pkgObj) {
                    var location;
                    pkgObj = "string" == typeof pkgObj ? { name: pkgObj } : pkgObj, location = pkgObj.location, pkgs[pkgObj.name] = { name: pkgObj.name, location: location || pkgObj.name, main: (pkgObj.main || "main").replace(currDirRegExp, "").replace(jsSuffixRegExp, "") }
                }), config.pkgs = pkgs), eachProp(registry, function(mod, id) { mod.inited || mod.map.unnormalized || (mod.map = makeModuleMap(id)) }), (cfg.deps || cfg.callback) && context.require(cfg.deps || [], cfg.callback)
            },
            makeShimExports: function(value) {
                function fn() { var ret; return value.init && (ret = value.init.apply(global, arguments)), ret || value.exports && getGlobal(value.exports) }
                return fn
            },
            makeRequire: function(relMap, options) {
                function localRequire(deps, callback, errback) { var id, map, requireMod; return options.enableBuildCallback && callback && isFunction(callback) && (callback.__requireJsBuild = !0), "string" == typeof deps ? isFunction(callback) ? onError(makeError("requireargs", "Invalid require call"), errback) : relMap && hasProp(handlers, deps) ? handlers[deps](registry[relMap.id]) : req.get ? req.get(context, deps, relMap) : (map = makeModuleMap(deps, relMap, !1, !0), id = map.id, hasProp(defined, id) ? defined[id] : onError(makeError("notloaded", 'Module name "' + id + '" has not been loaded yet for context: ' + contextName + (relMap ? "" : ". Use require([])")))) : (intakeDefines(), context.nextTick(function() { intakeDefines(), requireMod = getModule(makeModuleMap(null, relMap)), requireMod.skipMap = options.skipMap, requireMod.init(deps, callback, errback, { enabled: !0 }), checkLoaded() }), localRequire) }
                return options = options || {}, mixin(localRequire, {
                    ...
                    defined: function(id) { return hasProp(defined, makeModuleMap(id, relMap, !1, !0).id) },
                    specified: function(id) { return id = makeModuleMap(id, relMap, !1, !0).id, hasProp(defined, id) || hasProp(registry, id) }
    ...
    function getInteractiveScript() { return interactiveScript && "interactive" === interactiveScript.readyState ? interactiveScript : (eachReverse(scripts(), function(script) { if ("interactive" === script.readyState) return interactiveScript = script }), interactiveScript) }
...
function() {
    ...
    function c(a) {
        ...
        a: {
            ...
            for (var n = 0, H = g.length; n < H; n += 1)
                if (g[n] === a) { g = !0; break a }
            g = !1
        }
        g || (f.className += ("" === f.className ? "" : " ") + a)
    }

    function e(a, f, g) { this.b = null != a ? a : null, this.c = null != f ? f : null, this.g = null != g ? g : null }

    function k(a, f) { return 0 === a.compare(f) || 1 === a.compare(f) }
    ...
    function p() { var a = q; return 3 === a.a || 7 === a.a || 6 === a.a || 9 === a.a || 8 === a.a || 5 === a.a ? "grayscale" : 1 === a.a && k(a.d, new e(6, 2)) && 1 === a.e ? "grayscale" : "unknown" }
    var b = window,
        h = /^([0-9]+)(?:[\._-]([0-9]+))?(?:[\._-]([0-9]+))?(?:[\._+-]?(.*))?$/;
    e.prototype.compare = function(a) { return this.b > a.b || this.b === a.b && this.c > a.c || this.b === a.b && this.c === a.c && this.g > a.g ? 1 : this.b < a.b || this.b === a.b && this.c < a.c || this.b === a.b && this.c === a.c && this.g < a.g ? -1 : 0 };
